...
interface FeadbackSuccessStepProps {
  onFeadbackRestartRequested: () => void;
}

export function FeadbackSuccessStep({
  onFeadbackRestartRequested,
}: FeadbackSuccessStepProps) {
  return (
    <>
      <header>
        <CloseButton />
      ...
      <div className="flex flex-col items-center py-10 w-[304px]">
        ...
        <span className="text-xl mt-2">Agradecemos o feadback</span>

        <button
          onClick={onFeadbackRestartRequested}
          className="py-2 px-6 mt-6 bg-zinc-800 rounded-md border-transparent text-sm leading-6 hover:bg-slate-700 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-zinc-900 focus:border-brand-500"
        >
          Quero enviar outro
        </button>
      </div>
